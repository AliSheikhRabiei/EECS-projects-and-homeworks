...
class WatermainBreak:
    ...
    def __init__(self, when: date, year: int, lon: float, lat: float) -> None:
        if not isinstance(when, date):
            raise BreakException("Date must be a date object")
        if not isinstance(year, int):
            raise BreakException("Year must be an integer")
        if not isinstance(lon, float):
            raise BreakException("Longitude must be a float")
        if not isinstance(lat, float):
            raise BreakException("Latitude must be a float")
        if year < 1990 or year > 2016:
            raise BreakException("No data before 1990 or after 2016")
...
class BreakFilter:
    ...
    def __init__(self, filter_years: list[int], breaks: list[WatermainBreak]) -> None:
        ...
        self._filter_years = list(filter_years)
        if filter_years:
            self._breaks = [b for b in breaks if b.get_year() in filter_years]
        else:
...
def read_breaks_file(filename: str) -> list[WatermainBreak]:
    ...
    with open(filename, 'r') as f:
        reader = csv.reader(f)
        next(reader)  # Skip header (NEW)
        for row in reader:
            try:
                # CSV parsing logic (NEW)
                y, m, d = map(int, row[0].strip().split('/'))
                when = date(y, m, d)
                year = int(row[1].strip())
                lon = float(row[2].strip())
                lat = float(row[3].strip())
                breaks.append(WatermainBreak(when, year, lon, lat))
            except (ValueError, BreakException):
                continue  # Error handling (NEW)
    return breaks
