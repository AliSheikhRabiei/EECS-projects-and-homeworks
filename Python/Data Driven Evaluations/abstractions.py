...
class ApartmentBuilding:
    """An apartment, with a type (Private, TCHC, Social Housing), a location (or ward),
     a year of construction and and one or more city reviews."""
    _type: str
    _ward: int
    _year: int
    ...
    def apartment_min_rating(self) -> tuple[str, float]:
        ...
        if not self.reviews:
            return ("", 0.0)
        min_review = min(self.reviews, key=lambda r: r.rating)
        return (min_review.type, min_review.rating)
