...
public class StarterTests {
	...
	//adding some extra tests just in case they are needed or something comes up

	@Test
	public void test_log_03() {
	    Log appUpdate = new Log("5.7.31");
	    	    for (int i = 1; i <= 11; i++) {
	        appUpdate.addFix("Fix " + i);
	    }
	    
	    assertEquals("5.7.31", appUpdate.getVersion());
	    assertEquals(10, appUpdate.getNumberOfFixes());
	    assertEquals("[Fix 1, Fix 2, Fix 3, Fix 4, Fix 5, Fix 6, Fix 7, Fix 8, Fix 9, Fix 10]", appUpdate.getFixes());
	    assertEquals("Version 5.7.31 contains 10 fixes [Fix 1, Fix 2, Fix 3, Fix 4, Fix 5, Fix 6, Fix 7, Fix 8, Fix 9, Fix 10]", appUpdate.toString()); 

	    Log anotherUpdate = new Log("5.7.30");
	    assertNotEquals(appUpdate.getVersion(), anotherUpdate.getVersion());
	}

	@Test
	public void test_app_04() {
	    App app = new App("Test App", 3); 
	    for (int i = 1; i <= 21; i++) {
	        app.releaseUpdate("1." + i);
	    }  
	    assertEquals(20, app.getUpdateHistory().length);
	    assertEquals("Version 1.20 contains 0 fixes []", app.getWhatIsNew());

	    app.getUpdateHistory()[19].addFix("Final fix");
	    assertEquals("Version 1.20 contains 1 fixes [Final fix]", app.getWhatIsNew());
	    ...
	    assertEquals("Average of 3 ratings: 4.0 (Score 5: 1, Score 4: 1, Score 3: 1, Score 2: 0, Score 1: 0)", app.getRatingReport());

	    assertNull(app.getVersionInfo("2.0"));
	}

	@Test
	public void test_app_store_03() {

	    AppStore store = new AppStore("Test Store", 2); 
	    
	    App app1 = new App("App1", 5);
	    App app2 = new App("App2", 5);
	    ...
	    assertEquals(2, store.getStableApps(0).length); 
	    assertSame(app1, store.getApp("App1"));
	    assertSame(app2, store.getApp("App2"));
	    assertNull(store.getApp("App3"));

	    app1.releaseUpdate("1.0");
	    String[] stableApps = store.getStableApps(2);
	    assertEquals(0, stableApps.length);
	}

	@Test
	public void test_account_07() {

	    AppStore store = new AppStore("Test Store", 3);
	    App app1 = new App("App1", 5);
	    App app2 = new App("App2", 5);
	    App app3 = new App("App3", 5);
	    ...
	    Account acc = new Account("Tester", store);

	    acc.download("App1");
	    acc.download("App2");
	    acc.download("App3");
	    
	    assertEquals(3, acc.getObjectsOfDownloadedApps().length);
	    assertEquals("App3 is successfully downloaded for Tester.", acc.toString());

	    acc.download("App1"); 
	    assertEquals("Error: App1 has already been downloaded for Tester.", acc.toString());

	    acc.uninstall("App2");
	    assertEquals("App2 is successfully uninstalled for Tester.", acc.toString());

	    String[] names = acc.getNamesOfDownloadedApps();
	    assertEquals(2, names.length);
	    assertEquals("App1", names[0]);
	    assertEquals("App3", names[1]);

	    AppStore newStore = new AppStore("New Store", 10);
	    acc.switchStore(newStore);
	    assertEquals("Account for Tester is now linked to the New Store store.", acc.toString());

	    acc.submitRating("App1", 5);
	    assertEquals("Rating score 5 of Tester is successfully submitted for App1.", acc.toString());
	}
	//all paased!
}
